...
            audioRef.current.play().catch(e => console.log("Cần click vào màn hình để cấp quyền phát Mic:", e));
        }
    }, [stream]);
    return <audio ref={audioRef} autoPlay playsInline style={{ position: 'absolute', width: 0, height: 0, visibility: 'hidden' }} />;
};
...
  // ==========================================================
  useEffect(() => {
      // 1. Radar tự động quét trạng thái đăng nhập mỗi giây
...
          if (audio) {
              if (hasSession) {
                  // ĐÃ ĐĂNG NHẬP -> Tự động kích hoạt nhạc
                  if (audio.paused) {
                      audio.play().catch(() => {}); // Ẩn lỗi nếu trình duyệt quá gắt
                  }
              } else {
                  // CHƯA ĐĂNG NHẬP (Ở form) -> Tắt nhạc
                  if (!audio.paused) {
                      audio.pause();
                  }
              }
...
      // 2. Bẫy hỗ trợ: Nếu người chơi F5 tải lại trang (trình duyệt chặn)
      // thì chạm tay vào màn hình 1 cái là nhạc lên ngay.
      const unlockMusic = () => {
          const hasSession = localStorage.getItem('tienlen_user_session');
...
  // Điều chỉnh âm lượng: Nếu ở Sảnh thì to 100%, vô Game thì tụt xuống 15%
// ==========================================================
  const bgmGainNodeRef = useRef(null);
  const bgmSourceRef = useRef(null);

  useEffect(() => {
      // Móc thẻ <audio> vào hệ thống AudioContext để tước quyền kiểm soát của iOS
...
          if (audioCtx.state === 'suspended') audioCtx.resume(); // Ép iOS tỉnh dậy
          
          // Chỉnh âm lượng qua mạch trung gian, iOS không thể chặn
...
 // ==========================================================
  // 🟢 TỐI ƯU 1: Khóa bộ nhớ vị trí đối thủ & trạng thái trận đấu
  // Ngăn chặn việc chạy lại hàm này mỗi 1 giây gây tràn RAM
  // ==========================================================
  const realOpponents = React.useMemo(() => {
...
      
      return opponents;
  }, [roomPlayers, currentUser?.username]);
  
  useEffect(() => {
...
          });
      }
  }, [roomPlayers, currentUser, realOpponents]);

  const isEnded = gameState === 'ended';
  
  const pStatus = React.useMemo(() => ({
...
              }
              peer.ontrack = (event) => { setRemoteStreams(prev => ({ ...prev, [socketId]: event.streams[0] })); };
              peer.onicecandidate = (event) => {
                  if (event.candidate) socket.emit('webrtc_signal', { toSocketId: socketId, signal: { type: 'candidate', candidate: event.candidate } });
              };
...
                      peer.addTransceiver('audio', { direction: 'recvonly' });
                  }
                  peer.ontrack = (event) => { setRemoteStreams(prev => ({ ...prev, [fromSocketId]: event.streams[0] })); };
                  peer.onicecandidate = (event) => {
                      if (event.candidate) socket.emit('webrtc_signal', { toSocketId: fromSocketId, signal: { type: 'candidate', candidate: event.candidate } });
                  };
              }
...
  // ==========================================================
  // 🟢 TỐI ƯU 2: Đóng băng khu vực bài đánh ra giữa bàn (useMemo)
  // Chỉ vẽ lại lá bài khi có người đánh, KHÔNG vẽ lại theo đồng hồ đếm ngược
  // ==========================================================
...
      // ⚠️ MẸO GIẢM LAG: Nếu bàn có nhiều hơn 4 lượt đánh, chỉ hiển thị 4 lượt mới nhất, giấu bài cũ đi để giải phóng DOM
...
          )
      });
  }, [winMessage, tablePlays, currentUser?.username, realOpponents]);
  return (
    <>
        <audio 
            ref={globalAudioRef}
...
                />
                
                {/* POPUP CẢNH BÁO HẾT TIỀN (Chặn trước khi vào) */}
                {showBrokeModal && (
...
                {showSettings && (
                    <div style={{ position: 'absolute', top: '70px', left: '15px', background: 'rgba(0,0,0,0.9)', padding: '15px', borderRadius: '10px', border: '1px solid #f1c40f', zIndex: 101, color: 'white', boxShadow: '0 4px 15px rgba(0,0,0,0.5)' }}>
                        <h3 style={{ margin: '0 0 10px 0', color: '#f1c40f', textAlign: 'center' }}>CÀI ĐẶT GAME</h3>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                            <span style={{width: '70px'}}>🎵 Nhạc:</span>
...
                            <span style={{width: '70px'}}>🔊 SFX:</span>
...
                            <span>{sfxVolume}%</span>
                        </div>
                        <button onClick={() => setShowSettings(false)} style={{width: '100%', marginTop: '10px', padding: '5px', background: '#e74c3c', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer'}}>Đóng</button>
                    </div>
                )}
